...
final class QuantizerTables {
    ...
    /// Returns the active DC dequantizer after clamping one derived qindex.
    ///
    /// @param qIndex the derived qindex to clamp and resolve
    /// @param bitDepth the decoded sample bit depth
    /// @return the active DC dequantizer
    static int dcQuantizer(int qIndex, int bitDepth) {
        return tableValue(dcTable(bitDepth), qIndex);
    }

    /// Returns the active AC dequantizer after clamping one qindex.
    ///
    ...
    ///
    /// @param bitDepth the decoded sample bit depth
    /// @return the DC lookup table for the requested AV1 bit depth
    private static short[] dcTable(int bitDepth) {
        return switch (bitDepth) {
            case 8 -> DC_QLOOKUP_8_QTX;
            case 10 -> DC_QLOOKUP_10_QTX;
            case 12 -> DC_QLOOKUP_12_QTX;
            default -> throw new IllegalStateException("Unsupported quantizer table bit depth: " + bitDepth);
        };
    }

    /// Returns the AC lookup table for the requested AV1 bit depth.
    ///
    /// @param bitDepth the decoded sample bit depth
    /// @return the AC lookup table for the requested AV1 bit depth
    private static short[] acTable(int bitDepth) {
        return switch (bitDepth) {
            case 8 -> AC_QLOOKUP_8_QTX;
            case 10 -> AC_QLOOKUP_10_QTX;
            case 12 -> AC_QLOOKUP_12_QTX;
            default -> throw new IllegalStateException("Unsupported quantizer table bit depth: " + bitDepth);
        };
    }

    /// Returns one clamped table entry as an unsigned Java `int`.
    ///
    /// @param table the lookup table to index
    /// @param qIndex the derived quantizer index to clamp
    /// @return one clamped table entry as an unsigned Java `int`
    private static int tableValue(short[] table, int qIndex) {
        return table[clampedQIndex(qIndex)] & 0xFFFF;
    }
